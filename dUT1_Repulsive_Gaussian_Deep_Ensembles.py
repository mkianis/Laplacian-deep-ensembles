...
def weight_criterion(modelA, modelB, N_parameters):
    loss = sum([torch.sum(torch.exp(-torch.square(paramA.detach() - paramB))) for paramA, paramB in
                zip(modelA.parameters(), modelB.parameters())])
    return loss / N_parameters
...
for epoch_number in range(len(all_dates)):
    ...
    N_parameters = count_parameters(model_list[0])
    for iter in range(num_iter):
        for i in range(num_networks):
            ...
            for x_batch, y_batch in trainloader:
                ...
                loss_GDE = torch.mean(
                    0.5 * torch.log(sig_train) + 0.5 * torch.divide(torch.square(y_batch - mu_train), sig_train))
                loss_weight = sum(
                    [weight_criterion(model_list[i], model_list[j], N_parameters) for j in range(num_networks)])

                loss = loss_GDE - beta * loss_weight + CTE
    ...
    out_sig_final = np.sqrt(
        np.mean(np.square(out_sig), axis=0) + np.mean(np.square(out_mu), axis=0) - np.square(out_mu_final))
    ...
    np.savetxt(os.path.join(cwd, save_folder_name, f"mean_RGDE_{epoch_number + 1}.txt"), out_mu_final)
    np.savetxt(os.path.join(cwd, save_folder_name, f"std_RGDE_{epoch_number + 1}.txt"), out_sig_final)
