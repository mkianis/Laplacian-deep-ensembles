...
for epoch_number in range(len(all_dates)):
    ...
    for iter in range(num_iter):
        for i in range(num_networks):
            ...
            for x_batch, y_batch in trainloader:
                ...
                loss_LDE = torch.mean(torch.log(sig_train) + torch.divide(torch.abs(y_batch - mu_train), sig_train))
                loss_weight = sum(
                    [weight_criterion(model_list[i], model_list[j], N_parameters) for j in range(num_networks)])

                loss = loss_LDE - beta * loss_weight + CTE
    ...
    np.savetxt(os.path.join(cwd, save_folder_name, f"mean_RLDE_{epoch_number + 1}.txt"), out_mu_final)
    np.savetxt(os.path.join(cwd, save_folder_name, f"std_RLDE_{epoch_number + 1}.txt"), out_sig_final)
