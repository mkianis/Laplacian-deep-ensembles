...
for epoch_number in range(len(all_dates)):
    ...
    for iter in range(num_iter):
        for i in range(num_networks):
            ...
            for x_batch, y_batch in trainloader:
                ...
                loss = torch.mean(0.5 * torch.log(sig_train) + 0.5 * torch.divide(torch.square(y_batch - mu_train), sig_train)) + CTE
                tmp_loss += loss
    ...
    np.savetxt(os.path.join(cwd, save_folder_name, f"mean_GDE_{epoch_number+1}.txt"), out_mu_final)
    np.savetxt(os.path.join(cwd, save_folder_name, f"std_GDE_{epoch_number+1}.txt"), out_sig_final)
